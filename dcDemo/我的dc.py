...
with open(r'D:\PycharmProjects\demos\dcDemo\doneguild.json', 'r+', encoding='utf-8') as fp:
        ...
        if guild.name not in doneguild:
            ...
            for l in range(0, len(month)):
                if mday - alert[l] in (0, -1) and month[l] == mon:
                    ...
                    await bot.get_channel(993422944791429150).send(
                        guild.name + ' mint提醒\n' + djs + 'mint\n' + '\n'.join(price))
                    await bot.get_channel(1036569767236083712).send(
                        guild.name + ' mint提醒\n' + djs + 'mint\n' + '\n'.join(price))
...
async def on_message(message):
    ...
    if re.findall(r'submi', message.content, re.I) and re.findall(r'@everyone', message.content, re.I) and re.findall(
            r'addr|wallet', message.content, re.I):
        await push(message.guild.name + ' announcement',
                   'channel:\n' + message.channel.name + '\nauthor:\n' + message.author.name + '\ncontent:\n' + message.content)

    elif message.channel.name == 'bot':
        await qv(message.content)
    elif message.channel.id in [945930620230578246] and message.author.name != bot.user.name:
...
# p.daemon=True
# p.start()
bot.run('MTAwMTEyNDg1Nzg4MDI2NDcyNA.GLh1yG.lCxusrrbXh1-3NPt7IkjVR2z1H0e-ImJlT2qFM')
