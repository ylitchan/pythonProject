...
- `_get_oi_1d_data` 的可得边界是 `timestamp <= 当天UTC零点`，含当天那根未走完的日线（原有行为，本次只是把边界计算抽成函数）。1d 周期下这根要到明天零点才定型，作为切比雪夫区间外的近 10 根之一目前无影响，但 `max(sumOpenInterest_1d)` 会用到它。不在本次范围。
