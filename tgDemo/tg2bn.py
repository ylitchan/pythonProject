    ...
    async def handle_token(self, semaphore, symbol, success, dtn):
        ...
        try:
            ...
            if close_info := self.autobn.alert_all["POSITIONS"].get(symbol):
                ...
                if open_info[3] == "LONG" and time.time() - open_info[1] > 15 * 30 * 60:
                    self.autobn.alert_all["OBSERVATIONS"].pop(symbol)
                elif open_info[3] == "SHORT" and time.time() - open_info[1] > 15 * 60:
                    self.autobn.alert_all["OBSERVATIONS"].pop(symbol)
                else:
                    if (
                        open_info[3] == "LONG"
                        and kline_close[-1] > open_info[0]
                        and kline_close[-2] > kline_close[-3]
                        ...
                        zs = sum(kline_close[-7:]) / len(kline_close[-7:])
                        if kline_close[-1] >= zy:
                            return
...
async def main():
    ...
    for job in handle_msg.scheduler.get_jobs():
        print(
            f"  - {job.name}: {job.id}, 下次执行时间: {job.next_run_time}", flush=True
        )
    # 创建一个永不触发的事件，使程序一直运行
