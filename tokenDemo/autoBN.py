...
class AUTOBN:
    ...
    async def rzq_token(self, semaphore, symbol, success):
        ...
        try:
            ...
            if kline_close[-2] < max(kline_close[:-2]) and kline_close[-3] > min(kline_close[-5:-3]+[kline[-5][1], kline[-4][1]]) and kline_close[-3] < kline_close[-2] and max(kline[-3][5], kline[-4][5]) < kline[-2][5] and \
