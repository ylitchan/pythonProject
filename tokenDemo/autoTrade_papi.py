...
class AUTOBN:
    ...
    def from_cfg(cls, **kwargs):
        ...
        # 初始化币安统一账户(PAPI)客户端；公开行情仍沿用 UMFutures，减少改动面
        obj.market_client = UMFutures(key=api_key, secret=api_secret)
        papi_config = ConfigurationRestAPI(api_key=api_key, api_secret=api_secret)
        ...
        obj.market_client.session.mount("https://", adapter)
        obj.market_client.session.mount("http://", adapter)
    ...
    async def calculate_health_bn(self, notional, account_data=None) -> int:
        ...
        if account_data is None:
            account_data = await self._call_api(self.papi_client.rest_api.account_information)
    ...
    async def open_bn_position(
        ...
        try:
            account_data_raw, mark_price_data = await asyncio.gather(
                self._call_api(self.papi_client.rest_api.account_information),
                self._call_api(self.market_client.mark_price, symbol),
            )
    ...
    async def _get_oi_5m_data(self, symbol):
        ...
        oi_5m = await self._call_um(
            self.market_client.open_interest_hist,
            symbol=symbol,
            period="5m",
        ...
        if (
            ...
            try:
                data = await self._call_um(
                    self.market_client.long_short_account_ratio,
                    symbol=symbol,
                    period="1h",
    ...
    async def get_basis_rate(self, symbol: str) -> float:
        ...
        try:
            ...
            premium_index = await self._call_um(
                self.market_client.mark_price, symbol
            )
    ...
    def get_symbols_info(self, exchange_info=None):
        # 获取交易所信息（公开合约元信息继续沿用 UMFutures）
        if exchange_info is None:
            exchange_info = self.market_client.exchange_info()
        ...
        if now.minute % 15 == 0 or not self.symbols:
            for i in range(self.MAX_RETRY_COUNT):
                try:
                    ...
                    exchange_info = await self._call_api(self.market_client.exchange_info)
