...
def is_golden_cross(data, short_window=5, mid_window=10, long_window=20):
    ...
    # 判定是否满足多头排列条件
    return data[-2] <= data[-3] and not list(filter(
        lambda x: statistics.mean(data[x[0] + 1 - short_window:x[0] + 1]) > x[-1] if len(data) - 1 > x[0] > len(
            data) - 1 - short_window else False, enumerate(data)))
    #     and len(list(filter(
...
def rzq_token(symbol, alert, success):
    ...
    try:
        ...
        success.add(symbol)
        if price_f := [k[2] for k in kline[-5:-1] if k[4] <= k[1]]:
            price_f = max(price_f)
        else:
            return
        if zf >= 0 and price_close > price_f and is_golden_cross(kline_close):
