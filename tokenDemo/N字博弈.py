    ...
    if direction:
        for i in range(n):
            ...
            if not current_subseq or nums[i] > current_subseq[-1]:
                ...
                kk_asc = kk[:max(6 - len(current_subseq), 0)][::-1]
...
def rzq_token(symbol, alert, success):
    ...
    try:
        ...
        price_close_d = max([k[2] for k in kline[index_e + 1:-1]])
        if len(kline_close_asc) < 3 or price_close_d > price_close:
            return
        ...
        expectation = round((price_zy / price_close_d - 1) * 100, 2)
